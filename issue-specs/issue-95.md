...
- Unit coverage in `tests/unit/app-menu.test.ts`: the Sign out row is the first
  ...
  next free `U` number — the suite tops out at **U347**, so start at **U348**.
- The frozen hosted item-set test **E201** (`tests/e2e/hosted.spec.ts`, the
  ...
  added.
- One new e2e in `tests/e2e/hosted.spec.ts`, numbered **E214** (the suite tops
  out at E213), covers the flow end to end: signed in, the menu offers Sign
