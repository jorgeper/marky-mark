...
The directory provider (`server/providers/azure/graph.ts`) makes exactly five
...
(`User.ReadBasic.All`, `User.Invite.All` and `User.ReadWrite.All`), caching it
per user for its validity window. That exchange is what the client secret in
step 1.6 authenticates.
...
| `DELETE /users/{id}` | rescinding an invitation (Management → People's **Rescind** on a Pending row) — deletes the unredeemed guest's user object | `User.ReadWrite.All` |
...
guests, that restriction wins over the app's admin check. Rescinds run the
same way — the deletion is the admin's own Graph call, and the app only ever
issues it for a guest whose `externalUserState` is still `PendingAcceptance`;
members, accepted guests and admins are never deletable from the app, however
broad `User.ReadWrite.All` is.
...
`openid`, `profile`, `email`, `User.ReadBasic.All`, `User.Invite.All` and
`User.ReadWrite.All`. The first three are the sign-in scopes; listing them
keeps the consent screen honest even though Entra grants them implicitly.
...
`User.ReadBasic.All`, `User.Invite.All` and `User.ReadWrite.All` require
tenant-admin consent — an ordinary user cannot consent to them on first
sign-in. If your tenant
...
| `MM_AGENT_BRIDGE` | — (off) | optional | **Experimental.** `1` turns on the agent bridge: workspace administrators can mint, list and revoke workspace-scoped agent tokens from the app's Manage tab, the `/api/mcp` MCP endpoint answers those tokens, and a member who opts in to agent control opens a WebSocket control channel — which needs the App Service **Web sockets** switch on (the `--web-sockets-enabled true` step below). Unset, empty or `0` leaves it off — the routes do not exist and nothing about the deployment changes. Any other value refuses to start, naming the variable. Tokens are stored hashed and never logged. |
