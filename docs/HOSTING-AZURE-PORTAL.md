...
3. Tick these six (use the search box):
   - `openid`
   - `profile`
   - `email`
   - `User.ReadBasic.All`
...
top of the app's admin check. `User.ReadWrite.All` powers the matching
**Rescind** action (Management → People, on rows still badged **Pending**): it
deletes the unredeemed guest's user object as the signed-in admin. The app only
ever issues that deletion for a guest whose invitation is unaccepted — members,
accepted guests and admins are never deletable from the app, however broad the
permission reads.
...
`User.Invite.All` and `User.ReadWrite.All` are permissions an ordinary user
cannot consent to for themselves on first sign-in, so there is no way around
this — send the registration to whoever administers your tenant and have them
click it.
