...
/** The allowed set as the messages spell it: `.md, .markdown, …`. */
const allowedList = (): string => UPLOAD_EXTENSIONS.map((e) => `.${e}`).join(', ');

/**
 * PRD 007 Req 19: why this file's TYPE is refused, or null when it is
 * allowed. Split out because it is decidable from the name alone — the
 * server answers 415 from it before reading a single byte of the body.
 * The message names the offending extension AND the allowed set, because
 * "upload failed" tells the user nothing about what to do next.
 */
export function uploadTypeRejection(name: string): string | null {
  const ext = uploadExtension(name);
  if ((UPLOAD_EXTENSIONS as readonly string[]).includes(ext)) return null;
  return ext
    ? `“.${ext}” files cannot be uploaded — allowed: ${allowedList()}`
    : `“${name}” has no file extension — allowed: ${allowedList()}`;
}

/**
 * PRD 007 Req 19: why this upload is refused, or null when it is allowed —
 * the whole rule, size and type together. Size is judged first: a file that
 * breaks both rules is reported as oversize, the one failure the user cannot
 * work around by renaming.
 */
export function uploadRejection(name: string, size: number): string | null {
  ...
  return uploadTypeRejection(name);
}
