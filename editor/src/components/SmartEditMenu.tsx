...
export function SmartEditMenu({ x, y, entries, onInvoke, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const host = menuRef.current?.parentElement ?? null;
    for (let el = host; el; el = el.parentElement) remember(el);
    ...
    host?.querySelectorAll('.cm-scroller').forEach(remember);
    /** Still within SETTLE_PX of where this element stood when the menu opened. */
