...
test('E583: PRD 025 Reqs 9, 12, 14, 16 (issue #336) — with Fluid mode on, reduced motion draws no deletion ghost, a replacement or insertion never draws one, a deletion lands synchronously, a large removal draws nothing, Burst under reduced motion draws no particle, and Deletion → None draws nothing', async ({
  page,
}) => {
  await freshApp(page);
  await page.keyboard.press('Control+e');
