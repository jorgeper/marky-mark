...
test('E255: the toggleToc hotkey opens the sidebar on the TOC, hides it again, and is inert with no document open', async ({
  page,
}) => {
...
test('E256: with the folder tree showing, the hotkey switches the pane to the TOC in place, and Mod+Shift+E keeps its meaning', async ({
  page,
}) => {
  await seedFolders(page);
  await openFolderRoot(page);
...
test('E257: the sidebar reopens in the view it was left on — the TOC across a restart, folders unchanged', async ({
  page,
}) => {
  await openTree(page);
  await page.keyboard.press('Control+Shift+T');
