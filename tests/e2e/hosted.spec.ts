...
test('E366: People is its own settings tab, immediately after Editor, holding members, roles and the danger zone — and absent without a workspace', async ({
  page,
  request,
}) => {
...
test('E367: Add people autocompletes from the directory — guest badge, ↑/↓/Enter/Esc, and inline empty/error answers', async ({
  page,
  request,
}) => {
  // Issue #183 §3: the suggestions dropdown over the local mock directory.
  const ada = await signIn(request, 'ada');
  const id = await createWorkspace(request, ada, `E367 w${test.info().workerIndex}`);
...
test('E368: the Add people input and the role select share the one text-input rule — box, border, font and edges — in a light and a dark theme', async ({
  page,
  request,
}) => {
  ...
  const ada = await signIn(request, 'ada');
  const id = await createWorkspace(request, ada, `E368 w${test.info().workerIndex}`);
  await signInTo(page, 'ada', id);
  await openWorkspaceSettings(page);
