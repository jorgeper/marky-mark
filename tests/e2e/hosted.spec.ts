...
test('E187: an Owner adds a member through the settings picker, and that member can then open the workspace', async ({
  page,
  request,
}) => {
...
test('E188: changing a member’s role through the settings control changes what that member can do', async ({
  page,
  request,
}) => {
  ...
  const ada = await signIn(request, 'ada');
  const grace = await signIn(request, 'grace');
  const id = await sharedWorkspace(request, ada, `E188 w${test.info().workerIndex}`, [
...
test('E189: the last Owner cannot be demoted or removed — the refusal is visible and the server agrees', async ({
  page,
  request,
}) => {
  ...
  const ada = await signIn(request, 'ada');
  const id = await sharedWorkspace(request, ada, `E189 w${test.info().workerIndex}`, [
    { id: 'mock-grace', role: 'Editor' },
  ]);
...
test('E190: everyone-access grants its default role to non-members while an explicit member keeps their own', async ({
  page,
  request,
}) => {
  ...
  const grace = await signIn(request, 'grace');
  const alan = await signIn(request, 'alan');
  const id = await sharedWorkspace(request, ada, `E190 w${test.info().workerIndex}`, [
    { id: 'mock-grace', role: 'Viewer' },
  ]);
...
test('E191: a custom role created in the roles editor is grantable to a member and cannot be deleted while held', async ({
  page,
  request,
}) => {
  ...
  const id = await sharedWorkspace(request, ada, `E191 w${test.info().workerIndex}`, [
    { id: 'mock-grace', role: 'Viewer' },
  ]);
...
test('E192: a member without workspace.members or workspace.roles sees neither settings section', async ({
  page,
  request,
}) => {
