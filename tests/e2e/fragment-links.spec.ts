...
test('E509: a #fragment click in the full preview lands its heading at the pane top — below the click, deep down a long document, the right one of a duplicate pair, container-nested and h6 — and above it from the tail, without navigating', async ({
  page,
}) => {
...
test('E510: a #fragment matching no heading takes the PRD 020 Req 19 miss notice — the preview stays exactly where it is, dismissible, and nothing navigates', async ({
  page,
}) => {
  await openFragDoc(page);
  const doc = page.getByTestId('doc');
...
test('E511: the split view preview half honours the same managed-link rule — a #fragment lands in THAT half, an http(s) link hands off to the platform opener, a relative file stays inert, and the webview never navigates', async ({
  page,
}) => {
  await openFragDoc(page);
