...
test('E141: comments hidden (Mod+Shift+C) with the master switch ON — selection offers no button until shown again', async ({
  page,
}) => {
...
test('E142: an open composer suppresses a second Add-comment button until it closes', async ({ page }) => {
  await selectPhrase(page, PHRASE);
  await clickClearOfToolbar(page.getByTestId('add-comment-btn'));
  await expect(page.getByTestId('composer')).toBeVisible();
...
test('E143: plain edit mode reaches a comment — the affordance rides the SPEC25 carry and anchors the selected phrase', async ({
  page,
}) => {
...
test('E144: the edit-mode affordance obeys every gate — frozen store, hidden comments, master switch off', async ({
  page,
}) => {
...
test('E145: a native-menu install that REJECTS falls back to the in-app toolbar — never menu-less and toolbar-less', async ({
  page,
}) => {
