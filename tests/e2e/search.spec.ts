...
test('E548: in edit mode a result click selects EXACTLY the hit — before a gridded table, inside a table cell and below the table — and paints it in the find-active colours', async ({
  page,
}) => {
  await seedSearchTree(page);
...
test('E549: in preview mode a result click marks EXACTLY the hit words in the find-active colours — cross-file and same-file, in a table cell and inside inline markup — the row excerpt uses the find tint, and the mark clears on a query change', async ({
  page,
}) => {
  await seedSearchTree(page);
  await fsWrite(page, '/notes/table.md', TABLE_DOC);
