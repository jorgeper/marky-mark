...
// PRD 017 Req 1+25: the optional MM_ADMINS list — trimming, the whitespace
// refusal, and the local-mode default that azure mode deliberately lacks.
describe('PRD 017 Req 1+25 MM_ADMINS deployment admins', () => {
  const azure = {
    MM_MODE: 'azure',
    ENTRA_TENANT_ID: 't',
    ENTRA_CLIENT_ID: 'c',
...
describe('PRD 027 Req 2 MM_AGENT_BRIDGE', () => {
  ...
  it('U1383: unset, empty and 0 are off in both modes — the default every existing deployment is in', () => {
  ...
  it('U1384: 1 is on in both modes — local mode included, so the feature is developable against mock auth + Azurite', () => {
  ...
  it('U1385: any other value refuses to start naming the variable, rather than reading as on or off', () => {
