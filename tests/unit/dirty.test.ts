...
describe('Issue #42 dirty predicate', () => {
  ...
  it('U821: a CRLF-vs-LF-only difference is clean, in either direction', () => {
  ...
  it('U822: normalizeEol maps every ending to LF and leaves LF text alone', () => {
    expect(normalizeEol('a\r\nb\rc\nd')).toBe('a\nb\nc\nd');
    expect(normalizeEol('a\nb\nc')).toBe('a\nb\nc');
    expect(normalizeEol('')).toBe('');
  });

  it('U823: a parked table-mode buffer whose canonical form equals the saved text is clean', () => {
