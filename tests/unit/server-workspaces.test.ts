...
describe('PRD 007 Req 7+13 workspace API over HTTP', () => {
  ...
  beforeAll(async () => {
    server = createServer(
      ...
      // 404 is proven against its own app below (U1390).
      createApp(
        '/nonexistent-static',
  ...
  it('U1391: with the flag on, an Owner mints (201, plaintext once), lists (rows without it) and revokes (204; 404 for an unknown id)', async () => {
  ...
  it('U1392: every agent-token route needs workspace.settings — a member without it gets 403 naming the verb, and no session gets 401', async () => {
...
describe('PRD 027 Req 2 agent-token routes with the flag off', () => {
  ...
  it('U1390: mint, list and revoke all answer the ordinary 404 and write nothing', async () => {
