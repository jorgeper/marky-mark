...
describe('PRD 027 Req 8/12/13 (issue #366) agent bridge client', () => {
  it('U1418: every tool in BRIDGE_TOOL_NAMES answers a typed result echoing its id; the snapshot is filled from the injected sources', async () => {
    const ed = fakeEditor();
    const { executor } = ed;
  ...
  it('U1419: the revision is stable across cursor-only reports, changes with the content and with the path, and is never cached', async () => {
    const ed = fakeEditor();
  ...
  it('U1420: each mutating tool refuses a stale revision with the fresh state and no edit; a matching one edits and returns the new revision that get_editor_state then repeats', async () => {
  ...
  it('U1421: the text tools land through replaceRange with the caret after the insertion; the result reflects the live edit before the host commits, and reads dirty', async () => {
    const ed = fakeEditor();
  ...
  it('U1422: scroll by lines is relative to topLine, by pages uses the viewport measure, to a line is absolute, to a heading resolves through the section model; the caret never moves', async () => {
    const ed = fakeEditor();
    ed.select(4, 4);
  ...
  it('U1423: a null handle answers tool_failed naming the reason while get_editor_state still answers; open_file and save go through the registry callbacks; a throwing handle never escapes execute', async () => {
