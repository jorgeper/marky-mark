...
describe('PRD 027 Reqs 3+4 agent tokens', () => {
  it('U1398: mint answers the plaintext once and stores only a SHA-256 hash with {id, label, createdAt} under the workspace prefix', async () => {
  ...
  it('U1399: resolve succeeds for a live token and answers null for unknown, malformed and revoked ones — with no cache', async () => {
    const { provider, blobs } = createMemoryStorage();
  ...
  it('U1400: the scope check refuses another workspace and any path outside workspaces/<id>/ with the one stable error', () => {
  ...
  it('U1401: list rows carry id, label and createdAt only — never the plaintext or the hash — in creation order', async () => {
