...
export function createWorkspaceRepoView(inner: StorageProvider, id: string): StorageProvider {
  ...
  const wrap = (target: StorageProvider): StorageProvider => {
    ...
    // PRD 010 Req 6+7+12: forwarded, not re-implemented — `init()` is the
    // connected repo's own fail-fast writability check, `asUser` still yields
    // a view whose commits carry the acting user as author, and
    // `readAtVersion` keeps the merge capability alive through the mapping.
    // Each is forwarded ONLY when the wrapped provider has it, because their
    // absence is itself the contract: `server/workspaces.ts` decides "can this
    // workspace merge?" by whether the resolved provider offers
    // `readAtVersion`, so a view that dropped it would answer every stale
    // conditional save with a 412 — on precisely the git-backed workspaces
    // Req 12 exists for.
    const { init, asUser, readAtVersion } = target;
    if (init) view.init = () => init();
    if (asUser) view.asUser = (user: AuthUser) => wrap(asUser(user));
    // The version token addresses the bytes on its own; the mapped path is
    // what names the operation if the lookup fails.
    if (readAtVersion) view.readAtVersion = (path, version) => readAtVersion(at(path), version);
    return view;
  };
